...
- **Fixed faceted backdrop:** the decorative layer is `position:fixed; inset:0`
  so it stays put while the foreground (nav, greeting, cards, list, mission)
  scrolls over it — a quiet parallax. It is rendered **once at the app root**
  (`App.vue`) so it sits behind **every** page identically; the routed-content
  shell is lifted as a single block with `position:relative; z-index:1` (nav at
  `z-index:2`), and the backdrop sits at `z-index:0`.

---

## 6b. Faceted background system

A generated low-poly faceted surface is the shared signature backdrop across
**every** page of the app (home, welcome, voice, text, history, settings,
login). It is mounted once in `App.vue` (component: `FacetBackdrop.vue`) rather
than per-view, so it looks and behaves identically everywhere.

- **Form:** ~2,800 small jittered triangles (~26px) lit from the upper-left over
  a multi-bump height field, so they read as one dimensional dome rather than a
  flat pattern. The lit/shadow shading flows across all tiles into a single whole.
- **Tone:** monochrome only — **never multi-colored** (that reads as childish
  polka dots). Current treatment is **near-white**: fills ramp from a light warm
  gray shadow `rgb(222,218,210)` to pure white `#FFFFFF`, each tile given a faint
  warm edge `rgba(140,128,110,~0.16)` so the geometry stays legible on the cream
  surface (white "origami"). Prior treatments (warm-neutral taupe, charcoal-to-
  black) use the same generator with a different ramp.
- **Composition:** an organic ring/dome that **keeps clear of the page center**
  (so headings/text stay legible) and **dissolves into scattered tiles before the
  edges** (radial alpha falloff + random tile drop-out near the boundary), leaving
  a comfortable margin — it never tiles edge-to-edge across the whole screen.
- **Placement:** centered, `width: max(132vw, 160vh)` — a viewport "cover": the
  `132vw` term fills wide/landscape screens (incl. ultra-wide) seamlessly, while
  on tall/portrait screens (mobile) the height-driven `160vh` term wins so the
  dome fills the full height instead of shrinking to a mid-screen band. Sits
  inside the fixed backdrop layer. A single faint teal wash sits beneath for
  brand-secondary presence.
- **Source:** `frenchat-facets.svg`, regenerated by script (grid size, palette
  ramp, radius `rx/ry`, and falloff are the tunable parameters).
...
/* faceted backdrop (frenchat-facets.svg) */
--facet-asset:    url("frenchat-facets.svg");
--facet-tone:     near-white;   /* mono ramp: rgb(222,218,210) → #FFFFFF */
--facet-edge:     rgba(140,128,110,0.16);
--facet-layout:   position:fixed; centered; width:max(132vw,160vh) (cover: wide & tall); clear center; edge dissolve;
```

...
- **Placement:** centered, `width: 132vw` (scales with the viewport, no pixel
  cap, so the dome fills any screen size — including ultra-wide monitors —
  seamlessly), inside the fixed backdrop layer. A single faint teal wash sits
  beneath for brand-secondary presence.
...
--facet-layout:   position:fixed; centered; width:132vw (no cap, fills any screen); clear center; edge dissolve;
```
